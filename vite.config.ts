...
export default defineConfig(({ mode }) => ({
          ...
          (
            ...
            try {
              if (req.url === "/__env" && req.method === "GET") {
                ...
                (_res as ServerResponse).setHeader(
                  "Content-Type",
                  "application/json"
                );
              ...
              if (req.url === "/__log_client_env" && req.method === "POST") {
                ...
                req.on("end", () => {
                  try {
                    ...
                    console.log(
                      "[Env][Client posted] import.meta.env (masked):",
                      json
                    );
                  } catch (e) {
              ...
              if (req.url?.startsWith("/lab/ai-mockup")) {
                ...
                console.log(
                  "[Env][Request] VITE_GEMINI_API_KEY masked:",
                  mask(env.VITE_GEMINI_API_KEY)
                );
              }
            } catch {}
            next();
  ...
  test: {
    ...
    coverage: {
      ...
      include: ["src/pages/**/*.{ts,tsx}", "src/App.tsx", "src/main.tsx"],
      exclude: [
        "src/**/*.d.ts",
        "src/**/__tests__/**",
        "src/test/**",
